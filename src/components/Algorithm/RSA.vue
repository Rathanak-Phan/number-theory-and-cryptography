<template>
  <div
    class="max-w-xl mx-auto bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 border rounded-2xl shadow-lg p-6"
  >
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
      <div>
        ...
        <input
          type="number"
          v-model.number="p"
          class="w-full px-2 py-1 border rounded-md dark:bg-gray-700"
        />
      </div>
      <div>
      ...
      <div>
        <label class="block text-sm font-medium"
          >Message to Encrypt (A–Z):</label
        >
        <input
          type="text"
          v-model="userInput"
          class="uppercase w-full px-2 py-1 border rounded-md dark:bg-gray-700"
        />
      </div>
    </div>
    ...
    <div class="mb-4">
      <label class="block text-sm font-medium"
        >🔐 Ciphertext Blocks (space-separated):</label
      >
      <input
        v-model="manualCiphertext"
        placeholder="e.g. 2081 2182"
        class="w-full px-2 py-1 mt-1 border rounded-md dark:bg-gray-700 dark:text-white"
      />
      <button
        @click="decryptManual"
        class="mt-2 w-full bg-indigo-500 text-white py-2 rounded-lg hover:bg-indigo-600"
      >
    ...
    <pre
      class="mt-4 p-3 bg-gray-100 dark:bg-gray-800 rounded-lg border border-gray-300 dark:border-gray-700 whitespace-pre-wrap text-sm font-mono"
      >{{ result }}</pre
    >
  </div>
</template>
...
function decryptMessage() {
  ...
  const blockSize = getBlockSize(n.value);
  const decryptedDigits = ciphertextBlocks.value
    .map((c) => {
